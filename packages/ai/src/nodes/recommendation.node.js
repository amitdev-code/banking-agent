...
exports.createRecommendationNode = createRecommendationNode;
function buildRecommendations(scored, summary) {
    ...
    if (scored.qualifies && !scored.hasExistingLoan) {
        const spendRatio = summary.totalCreditLast12Months > 0
...
function createRecommendationNode(deps) {
    return async function recommendationNode(state) {
        deps.emitStep(state.runId, 'recommendation', 'running');
        const summaryMap = new Map(state.transactionSummaries.map((s) => [s.customerId, s]));
        const scoredWithRecs = state.scoredCustomers.map((scored) => {
