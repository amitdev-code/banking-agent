...
exports.createMessageNode = createMessageNode;
...
const messageSchema = zod_1.z.object({
    ...
    hindi: zod_1.z.string(),
});
function buildCustomerContext(scored, summary) {
    const avgSalary = summary.monthlySalaryCredits.filter((v) => v > 0).reduce((a, b) => a + b, 0) /
...
function createMessageNode(deps) {
    return async function messageNode(state) {
        ...
        if (paused) {
            (0, langgraph_1.interrupt)('Workflow paused by user before message generation');
        }
        ...
        for (const scored of qualifiedCustomers) {
            ...
            if (!summary)
                continue;
            const products = scored.recommendedProducts.map((r) => r.product);
            const ragContext = (0, retriever_1.buildRagContext)(products.length > 0 ? products : ['PERSONAL_LOAN']);
            const customerContext = buildCustomerContext(scored, summary);
            const result = await modelWithOutput.invoke([
                {
                    role: 'system',
                    content: 'You are a banking relationship manager generating personalized WhatsApp messages for loan products. ' +
