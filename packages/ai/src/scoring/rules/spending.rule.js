...
exports.SPENDING_MAX = void 0;
exports.spendingScore = spendingScore;
exports.SPENDING_MAX = 20;
const DIVERSE_CATEGORIES = [
    'GROCERY', 'FUEL', 'ENTERTAINMENT', 'DINING', 'SHOPPING',
    'UTILITIES', 'MEDICAL', 'TRAVEL', 'INSURANCE', 'INVESTMENT',
];
function spendingScore(summary) {
    let score = 0;
    // Diversity: how many non-EMI/ATM categories are present
    const presentCategories = new Set(summary.categoryTotals
        .filter((c) => c.type === 'DEBIT' && DIVERSE_CATEGORIES.includes(c.category))
        .map((c) => c.category));
    if (presentCategories.size >= 6)
        score += 10;
    else if (presentCategories.size >= 4)
        score += 7;
    else if (presentCategories.size >= 2)
        score += 4;
    else
        score += 1;
    ...
    return Math.min(score, exports.SPENDING_MAX);
}
